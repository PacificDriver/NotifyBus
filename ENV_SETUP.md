...
### API Перевозчика
> **⚠️ ВАЖНО:** Настройки API перевозчика теперь настраиваются через админ-панель (раздел "API Перевозчика").  
> Переменные окружения `CARRIER_API_*` больше не используются.

Настройки API перевозчика хранятся в базе данных с шифрованием:
- URL API перевозчика
...
2. Перейдите в раздел "Настройки" → "API Перевозчика"
3. Введите URL, ключ API и таймаут
...
> **⚠️ УСТАРЕЛО:** Эти переменные больше не используются.  
> Настройки API перевозчика теперь настраиваются через админ-панель и хранятся в базе данных.

~~- `CARRIER_API_URL` - URL API перевозчика~~ (теперь в БД)  
~~- `CARRIER_API_KEY` - Ключ доступа (x-access-token)~~ (теперь в БД с шифрованием)  
~~- `CARRIER_API_TIMEOUT` - Таймаут запросов в секундах~~ (теперь в БД)
